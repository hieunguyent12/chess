...
function Board({ board, turn, play_move, getLegalMoves }: BoardProps) {
    ...
    const onStartDragging = (e: React.MouseEvent, from: string, color: string) => {
        ...
        (e.currentTarget as HTMLElement).style.transform = `translate(${newTranslateX}px, ${newTranslateY}px)`;
    ...
    useEffect(() => {
        ...
        const onMouseUp = () => {
            ...
            if (legalMoves.filter(
                m => m.from == moveRef.current.from && m.to == moveRef.current.to).length > 0) {
