...
export class Chess {
  ...
  public move(move: Move, server: boolean) {
  ...
  public joinGame(game_id: string, player_name: string) {
    if (game_id === "" || this.socket_player_id === "") return;
  ...
  private connectToServer() {
    ...
    socket.addEventListener("message", (event) => {
      ...
      if (data.type === "connect") {
        this.socket_player_id = data.payload.id;
      }

      if (data.type === "opponentjoined") {
        let opponent = data.payload as Opponent;
        ...
        this.opponentConnectSubscribers.forEach((sub) => sub(opponent));
      }

      if (data.type === "makemove") {
        let move = data.payload as Move;
